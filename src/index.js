...
await page.goto(`https://${MASTODON_SERVER}/auth/sign_in`, {
  waitUntil: "load",
});
...
await page.goto(`https://${MASTODON_SERVER}/admin/trends/statuses`, {
  waitUntil: "load",
});
    ...
    await Promise.all(
      trends.map(async (trendId) => {
        const res = await fetch(
          `https://${MASTODON_SERVER}/api/v1/statuses/${trendId}`
        );
        ...
        if (isDisallowed) {
          ...
          console.log(
            `Filtering ${trendId} “${striptags(json.content).slice(
              0,
              50
            )}…” - ${json.url} `
          );
